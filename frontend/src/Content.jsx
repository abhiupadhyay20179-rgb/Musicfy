...
const Content = () => {
  ...
  const { playTrending, playStatus, play, pause, track, songsData } = useContext(PlayerContext);
  const songs = songsData && songsData[index] ? songsData[index] : null;
  ...
  if (!songs) {
    return (
      <div className="w-full min-h-[50vh] flex flex-col items-center justify-center text-white gap-3 p-6">
        ...
        <p className="text-neutral-400 text-sm font-medium">Loading song details...</p>
      </div>
    );
  }

  const isCurrentSong = track?.file === songs?.file && songs?.file !== '';
  const isPlaying = isCurrentSong && playStatus;

  const handlePlay = () => {
    if (!songs.file) return;
    if (isCurrentSong) {
  ...
  return (
    <div className='w-full min-h-full rounded-md pb-28 sm:pb-32 overflow-x-hidden'>
      <div
        ...
        style={{
          background: `linear-gradient(to bottom, ${songs.bgcolor || '#1a1a2e'}, #121212)`
        }}
      >
        <div className='flex flex-col sm:flex-row items-center sm:items-end gap-5 sm:gap-6 md:gap-8'>
          ...
          <div className='flex flex-col items-center sm:items-start text-center sm:text-left min-w-0 flex-1'>
            ...
            <p className='text-sm sm:text-base md:text-lg font-medium text-white/90 mt-2 truncate max-w-full'>
              {songs.desc}
              {songs.featuredArtist && (
                <span className='text-white/60 font-normal'> · {songs.featuredArtist}</span>
              )}
            </p>
      ...
      <div className='px-2 sm:px-4 md:px-6'>
        <div
          onClick={handlePlay}
          className={`flex items-center justify-between gap-3 sm:gap-4 py-2.5 sm:py-3 px-2 sm:px-4 rounded-lg transition-colors duration-150 group ${
            songs.file ? 'hover:bg-white/10 cursor-pointer' : 'opacity-50 cursor-not-allowed'
          } ${isCurrentSong ? 'bg-white/5' : ''}`}
        >
          <div className='flex items-center gap-3 sm:gap-4 min-w-0 flex-1'>
            <span className={`w-4 text-center text-xs sm:text-sm font-medium shrink-0 ${
              isCurrentSong ? 'text-[#1db954]' : 'text-neutral-400 group-hover:text-white'
            }`}>
              1
            </span>
            <img
